...
class Signal(object):
    def __init__(
        self, filename=None, mode='w-', driver='core', backing_store=False
    ):
        """
        ...
        *both* a filename and ``backing_store=True``.
        ...
        if filename is not None:
            try:
                ...
                if (
                    'file exists' in e.message
                    or 'Unable to open file' in e.message
                ):
                    error_msg = """\
...
3. Explicitly overwrite the file with mode='w'. Run:
    Signal('{0}', mode='w', driver={1}, backing_store={2})""".format(
        ...
        new_report.append(
            "HDF5 file {0} created in core memory".format(filename)
        )
    ...
    def load_nparray(
        self, s, s_name, s_unit, dt, s_help='cantilever displacement'
    ):
        ...
        new_report.append(
            "and duration {0:.3f} s".format(np.array(s).size * dt)
        )
    ...
    def plot(self, ordinate, LaTeX=False, component='abs'):
        ...
        if isinstance(y[0], complex) == True:
            ...
            if component == 'both':
                plt.plot(x, (np.array(y)).real)
                plt.plot(x, (np.array(y)).imag)
                y_label_string = "real and imag part of {}".format(
                    y_label_string
                )
    ...
    def time_mask_binarate(self, mode):
        ...
        dset = self.f.create_dataset(
            'workup/time/x_binarated', data=x_binarated
        )
        attrs = OrderedDict(
            [
                ('name', 't_masked'),
        ...
        new_report = []
        new_report.append(
            "Make an array, workup/time/mask/binarate, to be used"
        )
    ...
    def time_window_cyclicize(self, tw):
        ...
        new_report.append(
            "workup/time/window/cyclicize, with a rising/falling"
        )
    ...
    def time_mask_rippleless(self, td):
        ...
        **Programming notes**.  As a result of the applying cyclicizing window
        ...
        dset = self.f.create_dataset(
            'workup/time/x_rippleless', data=x_rippleless
        )
        attrs = OrderedDict(
            [
                ('name', 't_masked'),
    ...
    def fit_phase(self, dt_chunk_target):
        ...
        new_report.append(
            "frequency is {0:.3f} kHz.".format(1 / (2 * 1e3 * dt_chunk))
        )
        new_report.append(
        ...
        y_sub_reset = y_sub - y_sub[:, :, np.newaxis][:, 0, :] * np.ones(
            n_per_chunk
        )
        ...
        x_sub_reset = x_sub - x_sub[:, :, np.newaxis][:, 0, :] * np.ones(
            n_per_chunk
        )
        ...
        * From the ``lmfit`` documentation [`link <http://newville.github.io/lmfit-py/fitting.html#fit_report>`__]:    
        ...
        attrs = OrderedDict(
            [
                ...
                (
                    'label_latex',
                    '$a_{{\mathrm{{calc}}}} \: [\mathrm{{{0}}}]$'.format(
                        a_unit
                    ),
                ),
        ...
        dset = self.f.create_dataset(
            'workup/fit/exp/y_resid', data=result.residual
        )
        attrs = OrderedDict(
            [
                ('abscissa', y_dset.attrs['abscissa']),
                ...
                (
                    'label_latex',
                    '$a - a_{{\mathrm{{calc}}}} \: [\mathrm{{{0}}}]$'.format(
                        a_unit
                    ),
                ),
